...
def statistics_rewirings_clus(path,names,cut,nb_rewire,folder, folder_res, iter_pandemy, wl_it = -1,look_ahead = 1):
    ...
    if s not in files:
        fg = open(folder_res+s+"_clus.tex", "w")
        fg2 = open(folder_res+s+"_tri.tex", "w")
        for k in range(0,len(names)):
            ...
            col2 = read_dic(names_keep+"_"+str(1))
            nb = 1
            if nb != 0:
                m = len(g_new)
                clus = [[],[],[],[]]
                ...
                for z in range(nb_graphs):
                    ...
                    for ii in range(len(clus)):
                        ...
                        print("clust ok")
                        tri[ii].append(float(triangle_naive(to_aggregated(ll[ii]))))
                        print("tri ok")
                ...
                av_clus = [ sum(clus[ii])/nb_graphs  for ii in range(len(clus))]
                av_tri = [ sum(tri[ii])/nb_graphs  for ii in range(len(clus))]
                # av_eff_s = [ sum(eff_s[ii])/nb_graphs  for ii in range(3)]
                # av_eff_d = [ sum(eff_d[ii])/nb_graphs  for ii in range(3)]

                if nb_graphs > 1:
                    dev_clus = [ math.sqrt(sum( (clus[ii][jj]-av_clus[ii])**2 for jj in range(len(clus[ii]))  )/(nb_graphs - 1))  for ii in range(len(clus))]
                    dev_clus = [0] + dev_clus
                    dev_tri = [ math.sqrt(sum( (tri[ii][jj]-av_tri[ii])**2 for jj in range(len(tri[ii]))  )/(nb_graphs - 1))  for ii in range(len(clus))]
                    dev_tri = [0] + dev_tri
                    ...
                    dev_clus = [ 0  for ii in range(len(clus))]
                    dev_clus = [0] + dev_clus
                    dev_tri = [ 0  for ii in range(len(clus))]
                    dev_tri = [0] + dev_tri
